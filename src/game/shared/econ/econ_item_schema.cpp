...
#define daylight _daylight
	...
	m_bCustomItem = m_pKVItem->GetInt("customitem", 0) != 0; //OO//
...
,   m_mapCustomItems( DefLessFunc(int) ) //OO//
,	m_unVersion( 0 )
#if defined(CLIENT_DLL) || defined(GAME_DLL)
...
bool CEconItemSchema::BInitTextBuffer( CUtlBuffer &buffer, CUtlVector<CUtlString> *pVecErrors /* = NULL */ )
{
	...
	if (m_pKVRawDefinition->LoadFromFile(g_pFullFileSystem, "scripts/items/items_custom.txt", "GAME")) //OO//
	{ //@@//
		return BInitSchema( m_pKVRawDefinition, pVecErrors )
			&& BPostSchemaInit( pVecErrors );
	}
