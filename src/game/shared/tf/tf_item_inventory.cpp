...
CTFInventoryManager::~CTFInventoryManager( void )
{
	...
	m_pCustomLoadoutItems.PurgeAndDeleteElements(); //OO//
}
...
//-----------------------------------------------------------------------------
CEconItemView* CTFInventoryManager::AddCustomItem(int id) //OO//
{
	...
	m_pCustomLoadoutItems.AddToTail(pItemView); //OO//
	return pItemView;
}
...
void CTFInventoryManager::GenerateBaseItems(void)
{
	...
	// Add custom items
	const CEconItemSchema::BaseItemDefinitionMap_t& mapItemsCustom = GetItemSchema()->GetCustomItemDefinitionMap(); //OO//
	iStart = 0;
	if (mapItemsCustom.Count() != 0) //OO//
	{
		for (int it = iStart; it != mapItemsCustom.InvalidIndex(); it = mapItemsCustom.NextInorder(it)) //OO//
...
bool CTFInventoryManager::EquipItemInLoadout( int iClass, int iSlot, itemid_t iItemID )
{
	...
	if (iItemID < 100000)
	{
		int count = TFInventoryManager()->GetCustomItemCount(); //OO//
		for (int i = 0; i < count; i++ )
		{
			pItem = TFInventoryManager()->GetCustomItem(i); //OO//
...
int	CTFInventoryManager::GetAllUsableItemsForSlot( int iClass, int iSlot, CUtlVector<CEconItemView*> *pList )
{
	...
	iCount = m_pCustomLoadoutItems.Count(); //OO//
	for (int i = 0; i < iCount; i++)
	{
		CEconItemView* pItem = m_pCustomLoadoutItems[i]; //OO//
		CTFItemDefinition* pItemData = pItem->GetStaticData();
		if (!bIsAccountIndex && !pItemData->CanBeUsedByClass(iClass))
			continue;
		if (iSlot >= 0 && pItem->GetStaticData()->GetLoadoutSlot(iClass) != iSlot)
...
void CTFPlayerInventory::EquipLocal(uint64 ulItemID, equipped_class_t unClass, equipped_slot_t unSlot)
{
	...
	if (ulPreviousItem != 0 && ulPreviousItem < 100000)
	{
		...
		for (int i = 0; i < count; i++)
		{
			CEconItemView* pItem = TFInventoryManager()->GetCustomItem(i); //OO//
	...
	if (ulItemID < 100000)
	{
		...
		if (!pItem)
		{
			pItem = TFInventoryManager()->AddCustomItem(ulItemID); //OO//
			if (pItem && pItem->GetItemDefIndex() == ulItemID)
				pItem->GetSOCData()->Equip(unClass, unSlot);
		}
	}
...
CEconItemView *CTFPlayerInventory::GetItemInLoadout( int iClass, int iSlot )
{
	...
	else
	{
		...
		if ( m_LoadoutItems[iClass][iSlot] != LOADOUT_SLOT_USE_BASE_ITEM )
		{
			...
			if (m_LoadoutItems[iClass][iSlot] < 100000) //OO//
			{
